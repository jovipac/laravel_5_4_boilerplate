<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('username') }" v-if="type === 'username'">
   <input type="username" class="form-control" :placeholder="placeholder" :name="name" value="" v-model="form.username" @change="adddomain" autofocus/>
...
export default {
  ...
  data: function () {
    ...
    if (this.name === 'username') {
      form = new Form({ username: '', password: '', remember: '' })
    }
    return {
      form: form,
    }
  ...
  computed: {
    placeholder: function () {
      if (this.name === 'username') return this.trans('adminlte_lang_message.username')
